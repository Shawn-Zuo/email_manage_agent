...
- 将 `EmailActions` 集成到 `MailAgent`：  
  - 根据 `EmailAnalysis` 的 `category` / `priority`，自动对垃圾邮件或低优先级邮件执行“标记已读 / 移动到垃圾箱”等操作。  
- 提供 CLI 参数：  
  - 例如只查看高优先级邮件、只输出日报而不逐条打印、指定日期范围等。  
- 增加测试与示例：  
  - 为 `EmailService`、`ReportService`、`LLMService` 编写单元测试或集成测试。  
  - 在 README 中添加更多实际输出示例，便于快速理解运行效果。
